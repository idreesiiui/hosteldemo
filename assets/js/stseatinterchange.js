...
$(document).ready(function() {
    ...
    $("#newroomno").change(function() {
		var newroomno = $('#newroomno').val();
		var hostelno = $('#hostelid').val();
        //console.log(newroomno);
		hitURL = baseURL + "seatswap/interchange/getseatbyRoomId", 
		$.ajax({
            type:'POST',
			url : hitURL,
            data:{'newroomno':newroomno,'hostelno':hostelno},
			async: false,
            success:function(data){
				...
				if (data == '[]')
				{
					alert('Sorry! No Seat is Vacant against selected Room No');
				}
				var appenddata1 = "";
					...
					$('#newseat').html('<option value=""> Select VACANT SEAT</option>');
                    $("#newseat").append(appenddata1);
		...
		$.ajax({
            ...
            success:function(data){
				...
				var appenddata1 = "";
					...
					if(chostel != hostelno)
					{
						alert ('Oops! Swap Student Must be of your Block');
				    ...
				    alert ("Invalid Registration Number. Enter Regno Printed on Card");
					}
                // the next thing you want to do 
            }
