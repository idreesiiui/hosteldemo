...
$(document).ready(function() {
    ...
    $("#regno").change(function() {
		var regno = $('#regno').val();
       // console.log(regno);
		hitURL = baseURL + "reallotment/ReAllotment/VerifyUserRecord", 
		$.ajax({
            type:'POST',
			url : hitURL,
            data:{'regno':regno},
            ...
            success:function(data){
				...
				if (data != "[]")
				{
					if(data == 'true')
					{
						alert('Re-Allotement already Existed in Database or may be Black Listed');
					}
				var appenddata1 = "";
                    ...
                    for (var i = 0; i < jsonData1.length; i++) {
					   ...
					   var address = jsonData1[i].ADDRESS;
					   var preadd = jsonData1[i].CADDRESS;
					   var studentnumber = jsonData1[i].STUDENTPHONE;
					   var dob = jsonData1[i].STUDENTDOB;
					   var batchname = jsonData1[i].SEMESTERCODE;
					   var deptname = jsonData1[i].DEPARTNAME;
					   var programe = jsonData1[i].PROTITTLE;
					   ...
					   var email = jsonData1[i].EMAIL;
					   var emailid = jsonData1[i].EMAILID;
