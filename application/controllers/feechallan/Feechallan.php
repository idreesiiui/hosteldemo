 ...
 */
class Feechallan extends BaseController
{
    /**
...
$this->output->set_header("Expires: Mon, 26 Jul 1997 05:00:00 GMT");   
    ...
    {	
		...
		$data['viewHostelInfo'] = $this->feechallan_model->HostelDetail($gender);
		
		$this->global['pageTitle'] = 'IIUI Hostels : View Hostel Fee Details';
		
		$this->loadViews("feechallan/view", $this->global, $data, NULL);
        
    }
	
	function viewFeeChallanByStud()
	 {
		$userId = $this->vendorId; $studregno = $this->session->userdata('studregno');
		...
		if($allotment)
		  {
		   ...
		   //$uinfo = array('regno'=>$regno); 
		   
		   //$this->feechallan_model->UpdateUserRegno($gender,$userId, $uinfo);
		   
		//print_r($challaninfo);exit();
					...
					if($coursestype == 'ReAllotment' && $coursestatus == 1)
					   {
						...
						elseif(($protitle == 'MA' || $protitle == 'MSC') && ($nationality == 'Pakistani'))
						  {
							 $studTotalCredit = $this->Signup_model->getStudentCourseInfo($regno, $semcode);
	 ...
	 function viewFeeDetailByStudent()
    {
		...
		$genders = $this->common_model->GetGenderById($userId);
			
		$gender = $genders[0]->GENDER; 

		$userId = $genders[0]->userId;
...
/*		
	...
	function ViewNornalFeeChallanDetail($feeID = NULL)
    {
		...
		$paychllanid = $userinfo[0]->ID; 

		$regno = $userinfo[0]->REGNO; 

		$duedate = $userinfo[0]->CHALLANDUEDATE;
	...
	function challanNotFound($challaninfo)
    {
		...
		$this->global['pageTitle'] = 'IIUI Hostels : 404 - Challan Not Found 2';
            ...
            else
            {
				...
				if(!empty($existFeeChhallan))
				  ...
				  $challanno = $this->feechallan_model->GetLastChallanno();
					...
					$NewFeeDetailInforegno = array('REGNO'=>$regno, 'BATCHNAME'=>$bname, 'NATIONALITY'=>$nationality, 'PROTITTLE'=>$protittle, 'FEESTRUCSEM'=>$assignfeestruc, 'CHALLANNO'=>$lastchallanno, 'FINEAMOUNT'=>'0', 'FEETYPE'=>'NEW HOSTEL FEE', 'CURRENTSEMESTER'=>$csemester, 'GENDER'=>$gender, 'CHALLANDUEDATE'=>$duedate, 'STATUS'=>'1', 'updated_at'=>date('Y-m-d'), 'created_at'=>date('Y-m-d'));
   ...
   function SubmitNewFeeChallan()
    {	 
            ...
            else
            {
				...
				$data['reginfos'] = $reginfo; 
				$data['studnameinfo'] = $studnameinfo; 
				$data['bnameinfo'] = $bnameinfo; 
				$data['nationality'] = $nationality; 
				$data['programe'] = $programechallan; 
				$data['csem'] = $csemester; 
				$data['assignfee'] = $assignfeestruc; 
	...
	function hostelChallans(){
		
		$gender = $this->gender;
		
		$data['viewSemInfo'] = $this->feechallan_model->GetChallanSester($gender);
		
		$data['nationality'] = $this->feechallan_model->getChallanNationality($gender);

		$data['isPublish'] = $this->feechallan_model->getFeeChallanStatus($gender);

		$data['feeType'] = $this->feechallan_model->getFeeType($gender);

		//var_dump($data['feeType']);

		//exit();
		
		$this->global['pageTitle'] = 'IIUI Hostels : View Hostel Fee Details';
		
		$this->loadViews("feechallan/feeChallans", $this->global, $data, NULL);


	}

	function ViewHostelFeeChallans()
    {
      	$gender = $this->gender;
		...
		$data['viewHostelFeeChallans'] = $this->feechallan_model->ViewHostelFeeChallans($gender, $semester, $feetype, $ispublished, $nationality);			
		
		$this->global['pageTitle'] = 'IIUI Hostels : View Hostel Fee Details';
		
		$this->loadViews("feechallan/viewhostelFeechallans", $this->global, $data, NULL);
