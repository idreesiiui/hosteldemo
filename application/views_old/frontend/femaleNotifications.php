...
<div class="jumbotron jumbotron-md jumbotron-fluid mb-3 bg-light" style="background-image: url('<?php echo base_url();?>/assets/frontend/img/usis/headermale.png');background-size:cover;">
	<div class="container text-dark h-100 tofront">
		<div class="row align-items-center justify-content-center text-center text-white mt-5">
			<div class="col-md-10">
				<h1 class="display-3">Notifications <?php echo $campus; ?> Hostel</h1>
            </div>
		</div>
	</div>
</div>
...
<!-- table -->
<div class="bg-white">
<h2 class="bg-white text-success font-weight-bold text-center">Notifications</h2>
...
<tbody>
<?php
if($femaleNotificationInfo) {
$count = 1;
foreach($femaleNotificationInfo as $key => $list){
?>
<tr>
	<th scope="row"> <?php echo $count?></th>
	<td> <?php echo $list->upload_desc?> <small class="blink text-danger"style="font-size:12px; color:#9c27b0 !important"><b><?php if($count == 1) echo '(New)'?> </b></small> </td>
	<td> <?php echo date('d-m-Y', strtotime($list->pubdate))?> </td>
	<td><a href="<?php echo base_url();?><?php echo $list->path?>" target="_blank" rel="noopener noreferrer"><img src="<?php echo base_url();?>assets/frontend/img/usis/download.png" width=100 height=30 border="0" alt="Download"></a></td>
</tr>
<?php 
$count++;
   }
